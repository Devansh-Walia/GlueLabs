  ...
  render() {
    ...
    todos.forEach((element) => {
      ...
      if (element.completed === false) {
        // if the todo list item is not completed
    ...
    var main = (
      <div className="bottomContainer" >
        ...
        {left < todos.length ? (
          <button className="button clear" onClick={this.props.todoListHandler["clearCompleted"]} >Clear completed </button>
        ) : null}
      </div>
    );
    ...
    return (
      <>
        <div className="todo-list">
          {showing.length>0?<button className="completeAll button" onClick={this.props.todoListHandler["ToggleAll"]} >
